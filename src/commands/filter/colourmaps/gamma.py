...
from api.colourmaps import gamma as Filter
...
async def gamma(ctx: commands.Context, source: str = None, *flags, **priv) -> Any:
    ...
    if file.is_image:
        embed = discord.Embed(title="Gammafied image", colour=discord.Colour.red())
        embed.set_image(url=f"attachment://{file.filename}")
        m = await ctx.reply(embed=embed, file=file)
    else:
        m = await ctx.reply(content="Gammafied image data []".format(file.export_as), file=file)
...
ct!gamma [src...]? [--flags]
"""
COMMAND_DESCRIPTION = "Applies JET color map to image!"
