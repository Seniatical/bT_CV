...
from api.colourmaps import hsv as Filter
from core.com_par import parser
from binascii import Error


async def hsv(ctx: commands.Context, source: str = None, *flags, **priv) -> Any:
    ...
    if file.is_image:
        embed = discord.Embed(title="HSV image", colour=discord.Colour.red())
        embed.set_image(url=f"attachment://{file.filename}")
        m = await ctx.reply(embed=embed, file=file)
    else:
        m = await ctx.reply(content="HSV image data []".format(file.export_as), file=file)
...
COMMAND_CALLBACK = hsv
COMMAND_NAME = "hsv"
COMMAND_USAGE = """
ct!filter hsv [src...]?
ct!filter hsv [src...]? [--flags]
"""
COMMAND_DESCRIPTION = "Applies HSV color map an image!"
