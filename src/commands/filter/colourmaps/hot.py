...
from api.colourmaps import hot as Filter
from api.combine import combine
from core.com_par import parser
from binascii import Error


async def hot(ctx: commands.Context, source: str = None, *flags, **priv) -> Any:
    ...
    if file.is_image:
        embed = discord.Embed(title="Hot image", colour=discord.Colour.red())
        ...
        m = await ctx.reply(content="Hot image data []".format(file.export_as), file=file)
...
COMMAND_CALLBACK = hot
COMMAND_NAME = "hot"
COMMAND_USAGE = """
ct!filter hot [src...]?
ct!filter hot [src...]? [--flags]
"""
COMMAND_DESCRIPTION = "Applies HOT color map to an image!"
