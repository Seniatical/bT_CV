...
from api.colourmaps import viridis as Filter
from api.combine import combine
from core.com_par import parser
from binascii import Error


async def viridis(ctx: commands.Context, source: str = None, *flags, **priv) -> Any:
    ...
    if file.is_image:
        embed = discord.Embed(title="Viridis image", colour=discord.Colour.red())
        ...
        m = await ctx.reply(content="Viridis image data []".format(file.export_as), file=file)
...
COMMAND_CALLBACK = viridis
COMMAND_NAME = "viridis"
COMMAND_USAGE = """
ct!filter viridis [src...]?
ct!filter viridis [src...]? [--flags]
"""
COMMAND_DESCRIPTION = "Applies VIRIDIS color map to an image!"
