...
from api.colourmaps import turbo as Filter
from core.com_par import parser
from binascii import Error


async def turbo(ctx: commands.Context, source: str = None, *flags, **priv) -> Any:
    ...
    if file.is_image:
        embed = discord.Embed(title="Turbo image", colour=discord.Colour.red())
        embed.set_image(url=f"attachment://{file.filename}")
        m = await ctx.reply(embed=embed, file=file)
    else:
        m = await ctx.reply(content="Turbo image data []".format(file.export_as), file=file)
...
COMMAND_CALLBACK = turbo
COMMAND_NAME = "turbo"
COMMAND_USAGE = """
ct!filter turbo [src...]?
ct!filter turbo [src...]? [--flags]
"""
COMMAND_DESCRIPTION = "Applies TURBO color map to an image!"
