...
from api.colourmaps import cool as Filter
from api.combine import combine
from core.com_par import parser
from binascii import Error


async def cool(ctx: commands.Context, source: str = None, *flags, **priv) -> Any:
    ...
    if file.is_image:
        embed = discord.Embed(title="Cool image", colour=discord.Colour.red())
        ...
        m = await ctx.reply(content="Cool image data []".format(file.export_as), file=file)
...
COMMAND_CALLBACK = cool
COMMAND_NAME = "cool"
COMMAND_USAGE = """
ct!filter cool [src...]?
ct!filter cool [src...]? [--flags]
"""
COMMAND_DESCRIPTION = "Applies COOL color map to an image!"
