...
from api.colourmaps import inferno as Filter
from api.combine import combine
from core.com_par import parser
from binascii import Error


async def inferno(ctx: commands.Context, source: str = None, *flags, **priv) -> Any:
    ...
    if file.is_image:
        embed = discord.Embed(title="Inferno image", colour=discord.Colour.red())
        ...
        m = await ctx.reply(content="Inferno image data []".format(file.export_as), file=file)
...
COMMAND_CALLBACK = inferno
COMMAND_NAME = "inferno"
COMMAND_USAGE = """
ct!filter inferno [src...]?
ct!filter inferno [src...]? [--flags]
"""
COMMAND_DESCRIPTION = "Applies INFERNO color map to an image!"
