...
from api.colourmaps import deep_green as Filter
from api.combine import combine
from core.com_par import parser
from binascii import Error


async def deep_green(ctx: commands.Context, source: str = None, *flags, **priv) -> Any:
    ...
    if file.is_image:
        embed = discord.Embed(title="Deep Green image", colour=discord.Colour.red())
        ...
        m = await ctx.reply(content="Deep Green image data []".format(file.export_as), file=file)
...
COMMAND_CALLBACK = deep_green
COMMAND_NAME = "deep_green"
COMMAND_USAGE = """
ct!filter deep_green [src...]?
ct!filter deep_green [src...]? [--flags]
"""
COMMAND_DESCRIPTION = "Applies DEEP_GREEN color map to an image!"
