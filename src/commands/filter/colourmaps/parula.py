...
from api.colourmaps import parula as Filter
from api.combine import combine
from core.com_par import parser
from binascii import Error


async def parula(ctx: commands.Context, source: str = None, *flags, **priv) -> Any:
    ...
    if file.is_image:
        embed = discord.Embed(title="Parula image", colour=discord.Colour.red())
        ...
        m = await ctx.reply(content="Parula image data []".format(file.export_as), file=file)
...
COMMAND_CALLBACK = parula
COMMAND_NAME = "parula"
COMMAND_USAGE = """
ct!filter parula [src...]?
ct!filter parula [src...]? [--flags]
"""
COMMAND_DESCRIPTION = "Applies PARULA color map to an image!"
