...
from api.colourmaps import rainbow as Filter
from core.com_par import parser
from binascii import Error


async def rainbow(ctx: commands.Context, source: str = None, *flags, **priv) -> Any:
    ...
    if file.is_image:
        embed = discord.Embed(title="Rainbowed image", colour=discord.Colour.red())
        embed.set_image(url=f"attachment://{file.filename}")
        m = await ctx.reply(embed=embed, file=file)
    else:
        m = await ctx.reply(content="Rainbowed image data []".format(file.export_as), file=file)
...
COMMAND_CALLBACK = rainbow
COMMAND_NAME = "rainbow"
COMMAND_USAGE = """
ct!filter rainbow [src...]?
ct!filter rainbow [src...]? [--flags]
"""
COMMAND_DESCRIPTION = "Applies RAINBOW color map your image"
