...
from api.colourmaps import pink as Filter
from api.combine import combine
from core.com_par import parser
from binascii import Error


async def pink(ctx: commands.Context, source: str = None, *flags, **priv) -> Any:
    ...
    if file.is_image:
        embed = discord.Embed(title="Pink image", colour=discord.Colour.red())
        ...
        m = await ctx.reply(content="Pink image data []".format(file.export_as), file=file)
...
COMMAND_CALLBACK = pink
COMMAND_NAME = "pink"
COMMAND_USAGE = """
ct!filter pink [src...]?
ct!filter pink [src...]? [--flags]
"""
COMMAND_DESCRIPTION = "Applies PINK color map an image!"
