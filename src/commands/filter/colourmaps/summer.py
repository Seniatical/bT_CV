...
from api.colourmaps import summer as Filter
from api.combine import combine
from core.com_par import parser
from binascii import Error


async def summer(ctx: commands.Context, source: str = None, *flags, **priv) -> Any:
    ...
    if file.is_image:
        embed = discord.Embed(title="Summer image", colour=discord.Colour.red())
        ...
        m = await ctx.reply(content="Summer image data []".format(file.export_as), file=file)
...
COMMAND_CALLBACK = summer
COMMAND_NAME = "summer"
COMMAND_USAGE = """
ct!filter summer [src...]?
ct!filter summer [src...]? [--flags]
"""
COMMAND_DESCRIPTION = "Applies SUMMER color map to an image!"
