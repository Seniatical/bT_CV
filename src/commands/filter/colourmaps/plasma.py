...
from api.colourmaps import plasma as Filter
from core.com_par import parser
from binascii import Error


async def plasma(ctx: commands.Context, source: str = None, *flags, **priv) -> Any:
    ...
    if file.is_image:
        embed = discord.Embed(title="Plasma image", colour=discord.Colour.red())
        embed.set_image(url=f"attachment://{file.filename}")
        m = await ctx.reply(embed=embed, file=file)
    else:
        m = await ctx.reply(content="Plasma image data []".format(file.export_as), file=file)
...
COMMAND_CALLBACK = plasma
COMMAND_NAME = "plasma"
COMMAND_USAGE = """
ct!filter plasma [src...]?
ct!filter plasma [src...]? [--flags]
"""
COMMAND_DESCRIPTION = "Applies PLASMA color map to an image!"
