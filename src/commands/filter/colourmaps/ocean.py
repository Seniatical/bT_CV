...
from api.colourmaps import ocean as Filter
from api.combine import combine
from core.com_par import parser
from binascii import Error


async def ocean(ctx: commands.Context, source: str = None, *flags, **priv) -> Any:
    ...
    if file.is_image:
        embed = discord.Embed(title="Ocean image", colour=discord.Colour.red())
        ...
        m = await ctx.reply(content="Ocean image data []".format(file.export_as), file=file)
...
COMMAND_CALLBACK = ocean
COMMAND_NAME = "ocean"
COMMAND_USAGE = """
ct!filter ocean [src...]?
ct!filter ocean [src...]? [--flags]
"""
COMMAND_DESCRIPTION = "Applies OCEAN color map to an image!"
