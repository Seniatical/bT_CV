...
from api.colourmaps import magma as Filter
from api.combine import combine
from core.com_par import parser
from binascii import Error


async def magma(ctx: commands.Context, source: str = None, *flags, **priv) -> Any:
    ...
    if file.is_image:
        embed = discord.Embed(title="Magma image", colour=discord.Colour.red())
        ...
        m = await ctx.reply(content="Magma image data []".format(file.export_as), file=file)
...
COMMAND_CALLBACK = magma
COMMAND_NAME = "magma"
COMMAND_USAGE = """
ct!filter magma [src...]?
ct!filter magma [src...]? [--flags]
"""
COMMAND_DESCRIPTION = "Applies MAGMA color map to an image!"
