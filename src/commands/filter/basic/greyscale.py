...
from api.effects import greyscale as F
from api.combine import combine
from core.com_par import parser
from binascii import Error


async def greyscale(ctx: commands.Context, source: str = None, *flags, **priv) -> Any:
    ...
    if file.is_image:
        embed = discord.Embed(title="Greyscaled image", colour=discord.Colour.red())
        ...
        m = await ctx.reply(content="Greyscaled image data []".format(file.export_as), file=file)
...
COMMAND_CALLBACK = greyscale
COMMAND_NAME = "greyscale"
COMMAND_USAGE = """\n
ct!filter greyscale [src...]?
ct!filter greyscale [src...]? [--flags]
"""
COMMAND_DESCRIPTION = "Greyscales an image!"
COMMAND_BRIEF = COMMAND_DESCRIPTION
COMMAND_HELP = COMMAND_DESCRIPTION
COMMAND_ALIASES = ["grayscale"]
