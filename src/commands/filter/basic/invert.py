...
from api.effects import invert as fil
from core.com_par import parser
from binascii import Error


async def invert(ctx: commands.Context, source: str = None, *flags, **priv) -> Any:
    ...
    def filter():
        return fil(stream, animate=prsed["animate"], frame=prsed["frame"])
    ...
    if file.is_image:
        embed = discord.Embed(title="Inverted image", colour=discord.Colour.red())
        embed.set_image(url=f"attachment://{file.filename}")
        m = await ctx.reply(embed=embed, file=file)
    else:
        m = await ctx.reply(content="Inverted image data []".format(file.export_as), file=file)
...
COMMAND_CALLBACK = invert
COMMAND_NAME = "invert"
COMMAND_USAGE = """
ct!filter invert [src...]?
ct!filter invert [src...]? [--flags]
"""
COMMAND_DESCRIPTION = "Invert an image's colour!"
