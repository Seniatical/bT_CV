...
from api.effects import solarize as F
from core.com_par import parser
from binascii import Error


async def solarize(ctx: commands.Context, source: str = None, threshold: int = 128, *flags, **priv) -> Any:
    ...
    def filter():
        return F(stream, threshold, animate=prsed["animate"], frame=prsed["frame"])
    ...
    if file.is_image:
        embed = discord.Embed(title="Solarized image", colour=discord.Colour.red())
        embed.set_image(url=f"attachment://{file.filename}")
        m = await ctx.reply(embed=embed, file=file)
    else:
        m = await ctx.reply(content="Solarized image data []".format(file.export_as), file=file)
...
COMMAND_CALLBACK = solarize
COMMAND_NAME = "solarize"
COMMAND_USAGE = """\n
ct!filter solarize [src...]? [threshold:int=128]
ct!filter solarize [src...]? [threshold:int=128] [--flags]
"""
COMMAND_DESCRIPTION = "Solarize an image!"
