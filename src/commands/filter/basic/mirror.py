...
async def mirror(ctx: commands.Context, source: str = None, *flags, **priv) -> Any:
    ...
    if file.is_image:
        embed = discord.Embed(title="Mirrored image", colour=discord.Colour.red())
        ...
        m = await ctx.reply(content="Mirrored image data []".format(file.export_as), file=file)
...
COMMAND_CALLBACK = mirror
COMMAND_NAME = "mirror"
COMMAND_USAGE = """
ct!filter mirror [src...]?
ct!filter mirror [src...]? [--flags]
"""
COMMAND_DESCRIPTION = "Mirrors an image!"
