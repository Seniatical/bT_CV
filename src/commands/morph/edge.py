...
from api.morph import edge_morph

from binascii import Error
from api.loader import SourceNotFound, FileTooLarge


async def edge(ctx: Context, source: str = None, *flags) -> Any:
    ...
    def applyMorph():
        return edge_morph(stream, **prsed)
    ...
    if file.is_image:
        embed = Embed(title="Morphed (EDGE) image", colour=Colour.red())
        ...
        m = await ctx.reply(content="Morphed (EDGE) image data []".format(file.export_as), file=file)
...
COMMAND_CALLBACK = edge
COMMAND_NAME = "edge"
COMMAND_USAGE = """
ct!morph edge [src...]?
ct!morph edge [src...]? [--flags]
"""
COMMAND_DESCRIPTION = "Morph an image using the edge option!"
