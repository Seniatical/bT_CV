...
from api.morph import dilation8_morph

from binascii import Error
from api.loader import SourceNotFound, FileTooLarge


async def dilation8(ctx: Context, source: str = None, *flags) -> Any:
    ...
    def applyMorph():
        return dilation8_morph(stream, **prsed)
    ...
    if file.is_image:
        embed = Embed(title="Morphed (DILATION8) image", colour=Colour.red())
        ...
        m = await ctx.reply(content="Morphed (DILATION8) image data []".format(file.export_as), file=file)
...
COMMAND_CALLBACK = dilation8
COMMAND_NAME = "dilation8"
COMMAND_USAGE = """
ct!morph dilation8 [src...]?
ct!morph dilation8 [src...]? [--flags]
"""
COMMAND_DESCRIPTION = "Morph an image using the dilation8 option!"
