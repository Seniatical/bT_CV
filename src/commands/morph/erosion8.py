...
from api.morph import erosion8_morph

from binascii import Error
from api.loader import SourceNotFound, FileTooLarge


async def erosion8(ctx: Context, source: str = None, *flags) -> Any:
    ...
    def applyMorph():
        return erosion8_morph(stream, **prsed)
    ...
    if file.is_image:
        embed = Embed(title="Morphed (EROSION8) image", colour=Colour.red())
        ...
        m = await ctx.reply(content="Morphed (EROSION8) image data []".format(file.export_as), file=file)
...
COMMAND_CALLBACK = erosion8
COMMAND_NAME = "erosion8"
COMMAND_USAGE = """
ct!morph erosion8 [src...]?
ct!morph erosion8 [src...]? [--flags]
"""
COMMAND_DESCRIPTION = "Morph an image using the erosion8 option!"
