...
from api.morph import corner_morph

from binascii import Error
from api.loader import SourceNotFound, FileTooLarge


async def corner(ctx: Context, source: str = None, *flags) -> Any:
    ...
    def applyMorph():
        return corner_morph(stream, **prsed)
    ...
    if file.is_image:
        embed = Embed(title="Morphed (CORNER) image", colour=Colour.red())
        ...
        m = await ctx.reply(content="Morphed (CORNER) image data []".format(file.export_as), file=file)
...
COMMAND_CALLBACK = corner
COMMAND_NAME = "corner"
COMMAND_USAGE = """
ct!morph corner [src...]?
ct!morph corner [src...]? [--flags]
"""
COMMAND_DESCRIPTION = "Morph an image using the corner option!"
