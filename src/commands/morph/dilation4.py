...
from api.morph import dilation4_morph

from binascii import Error
from api.loader import SourceNotFound, FileTooLarge


async def dilation4(ctx: Context, source: str = None, *flags) -> Any:
    ...
    def applyMorph():
        return dilation4_morph(stream, **prsed)
    ...
    if file.is_image:
        embed = Embed(title="Morphed (DILATION4) image", colour=Colour.red())
        ...
        m = await ctx.reply(content="Morphed (DILATION4) image data []".format(file.export_as), file=file)
...
COMMAND_CALLBACK = dilation4
COMMAND_NAME = "dilation4"
COMMAND_USAGE = """
ct!morph dilation4 [src...]?
ct!morph dilation4 [src...]? [--flags]
"""
COMMAND_DESCRIPTION = "Morph an image using the dilation4 option!"
