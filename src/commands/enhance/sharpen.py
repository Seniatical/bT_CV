...
from api.enhance import sharpen
from api.combine import combine
from core.com_par import parser


async def sharp(ctx: Context, source: str = None, factor: float = 1, *flags) -> Any:
    ...
    def applyEnhancer():
        return sharpen(stream, factor, **prsed)
    ...
    if file.is_image:
        embed = Embed(title="Sharpen adjusted image", colour=Colour.red())
        ...
        m = await ctx.reply(content="Sharpen adjusted image data []".format(file.export_as), file=file)
...
COMMAND_CALLBACK = sharp
COMMAND_NAME = "sharpen"
COMMAND_DESCRIPTION = "Adjust image sharpness"
COMMAND_BRIEF = COMMAND_DESCRIPTION
COMMAND_HELPS = COMMAND_DESCRIPTION
COMMAND_USAGE = """
ct!enhance sharpen [source:string]? [factor:float=1.0[SAME]]?
ct!enhance sharpen [source:string] [factor:float=1.0[SAME]] [--flags]
"""
