...
from api.enhance import color
from api.combine import combine
from core.com_par import parser


async def colour(ctx: Context, source: str = None, factor: float = 1, *flags) -> Any:
    ...
    def applyEnhancer():
        return color(stream, factor, **prsed)
    ...
    if file.is_image:
        embed = Embed(title="Colour balanced image", colour=Colour.red())
        ...
        m = await ctx.reply(content="Colour balanced image data []".format(file.export_as), file=file)
...
COMMAND_NAME = "colour"
COMMAND_DESCRIPTION = "Adjust image color balance"
COMMAND_BRIEF = COMMAND_DESCRIPTION
COMMAND_HELPS = COMMAND_DESCRIPTION
COMMAND_USAGE = """
ct!enhance colour [source:string]? [factor:float=1.0[SAME]]?
ct!enhance colour [source:string] [factor:float=1.0[SAME]] [--flags]
"""
