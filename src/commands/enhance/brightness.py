...
from api.enhance import brightness
from api.combine import combine
from core.com_par import parser


async def bright(ctx: Context, source: str = None, factor: float = 1, *flags) -> Any:
    ...
    def applyEnhancer():
        return brightness(stream, factor, **prsed)
    ...
    if file.is_image:
        embed = Embed(title="Brightness adjusted image", colour=Colour.red())
        ...
        m = await ctx.reply(content="Brightness adjusted image data []".format(file.export_as), file=file)
...
COMMAND_CALLBACK = bright
COMMAND_NAME = "brightness"
COMMAND_DESCRIPTION = "Adjust image brightness"
COMMAND_BRIEF = COMMAND_DESCRIPTION
COMMAND_HELPS = COMMAND_DESCRIPTION
COMMAND_USAGE = """
ct!enhance brightness [source:string]? [factor:float=1.0[SAME]]?
ct!enhance brightness [source:string] [factor:float=1.0[SAME]] [--flags]
"""
